...
url_db = "https://api.open-meteo.com/v1/meteofrance?latitude=43.6109&longitude=3.8763&daily=weathercode,temperature_2m_max,temperature_2m_min,precipitation_sum,windspeed_10m_max,winddirection_10m_dominant&timezone=GMT&start_date={0}&end_date={1}&format=csv".format(
    aujourd_hui, J_plus_4
)
...
url_db1 = "https://api.open-meteo.com/v1/meteofrance?latitude=43.6109&longitude=3.8763&hourly=windspeed_10m&start_date={0}&end_date={1}&format=csv".format(
    aujourd_hui, J_plus_4
)
...
noms_jours = ["lundi", "mardi", "mercredi",
...
for i in range(ncols):
    ...
    if type(pluie[i]) is float and pluie[i] >= 0:
...
for i in range(ncols):
    if type(df.iloc[6, i]) is float and df.iloc[6, i] >= 0:
